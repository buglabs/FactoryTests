...
pim()
{
	...
	echo "Checking if mPIM$1 is plugged in"
	check_pim $1
	if [ $? = "1" ];then
		return
	fi
	...
	if [ $1 = "2" ];then
		...
		echo "LCD PIM TESTS" >> /home/testlog.txt
		echo "=============" >> /home/testlog.txt
		echo "---------------------"
		echo "+---LCD PIM Tests---+"
		echo "---------------------"
...
#------------------------------------------------------------------------------
		...
		echo
		echo "" >> /home/testlog.txt
		echo "TEST PIM$1 TESTS" >> /home/testlog.txt
		echo "==============" >> /home/testlog.txt
		echo "----------------------"
		echo "+---TEST PIM$1 Tests---+"
		echo "----------------------"
		echo
		...
		echo
		echo "Starting SPI EEPROM Read/Write Test"
		echo
		spi_eep_w $1
		echo
		spi_eep_r $1
		if [ "$?" == "0" ];then
			echo Test Passed
			echo PIM$1 SPI EEPROM Read/Write Test Passed >> /home/testlog.txt
		else
			echo Test Failed
			echo PIM$1 SPI EEPROM Read/Write Test Failed >> /home/testlog.txt
			fi
		echo
		echo "Starting McBSP Test"
		echo
		retry=0
		while [ $retry -le $RETRYMAX ] ; do
			mcbsp $1
			mcbsp_count=$(( $mcbsp_count + 1 ))
			sleep 2
			dmesg | grep -i Read | tail -n 1 > mcbspresult
			cmp /usr/tests/mcbspcontrol mcbspresult
			if [ "$?" == "0" ];then
				echo Test Passed
				echo PIM$1 McBSP Test Passed >> /home/testlog.txt
				break
			else
				echo Test Failed
				if [ $retry -lt $RETRYMAX ]; then
					echo PIM$1 McBSP Test Failed, retrying >> /home/testlog.txt
				else
					echo PIM$1 McBSP Test Failed, retrying >> /home/testlog.txt
				fi
			fi
		...
		if [ $1 -eq 4 ];then
				...
				if [ $bat_detect -eq "1" ];then
					cat /sys/class/i2c-adapter/i2c-3/3-0055/battery_init
				else
					echo "Battery init FAILED"
					echo "PIM$1 Battery Test failed" >> /home/testlog.txt
					return
				fi
				echo "Initializing Battery... Please wait"
				sleep 10
					pim_battery
				echo "Is the battery values valid?(y/n)"
				resp=y
				if [ $resp = "y" ];then
					echo Test Passed
					echo PIM$1 Battery Test Passed >> /home/testlog.txt
				else
					echo Test Failed
					echo PIM$1 Battery Test Failed >> /home/testlog.txt
				fi
...
if [ -b /dev/mmcblk1p1 ];then
	...
	echo 2 > /sys/bus/spi/drivers/SC16IS762/spi1.0/i2c_sw_base_en
	pim 2
	echo 3 > /sys/bus/spi/drivers/SC16IS762/spi1.0/i2c_sw_base_en
	pim 3
	echo 4 > /sys/bus/spi/drivers/SC16IS762/spi1.0/i2c_sw_base_en
	pim 4

	interactive_tests
	umount /dev/mmcblk1p1
	mount -t vfat /dev/mmcblk1p1 /mnt/mmc2/
	echo "Copying log file name to User SD ..."
	cp /home/testlog.txt /mnt/mmc2/$log_file_name
	umount /mnt/mmc2
	...
	echo "==========================================================================="
	echo
	grep -i fail /home/testlog.txt
	echo
	echo "==========================================================================="
	echo " Press Control + ]  to proceed to the next steps"
	echo "==========================================================================="

else
	echo "Insert User SD card and run the test"
	echo
fi
#End of File
